...
public class CMSNSManager {
	...
	public static ArrayList<String> getFriendList(String strUserName, CMInfo cmInfo)
	{
		...
		friendList = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryGetFriendsList(strUserName, cmInfo);
	...
	public static ArrayList<String> getFriendRequesterList(String strUserName, CMInfo cmInfo)
	{
		...
		candidateList = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryGetRequestersList(strUserName, cmInfo);
		
		// get users whom I added as friends
		myFriendList = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryGetFriendsList(strUserName, cmInfo);
	...
	protected static void checkCompleteRecvAttachedFiles(CMFileEvent fe, CMInfo cmInfo)
	{
		...
		if(confInfo.getSystemType().equals("SERVER"))
		{
			...
			if(confInfo.isDBUse())
			{
				kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryInsertSNSAttachedFile(nContentID, strFilePath, strFileName, cmInfo);
			}
			else
			{
	...
	public static void loadAccessHistory(CMUser user, CMInfo cmInfo)
	{
		...
		historyList = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryGetAccessHistory(strUserName, startDate, endDate, null, cmInfo);
	...
	public static void saveAccessHistory(CMUser user, CMInfo cmInfo)
	{
		...
		for(int i = 0; i < arrayList.size(); i++)
		{
			...
			{				
				...
				nRet = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryInsertAccessHistory(strUserName, date, strWriterName, nAccessCount, cmInfo);
			...
			else if(tempHistory.isUpdated())
			{
				...
				nRet = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryUpdateAccessCount(strUserName, date, strWriterName, nAccessCount, cmInfo);
				if(nRet == 1) // DB query succeeded
				{
					nUpdated++;
				}
			...
			if(isServerPrefetchOngoing(strUser, cmInfo))
			{
				if(CMInfo._CM_DEBUG)
					System.out.println("CMSNSManager.processCONTENT_DOWNLOAD_REQUEST(); previous prefetch ongoing "
							+ "to the user("+strUser+")");
				kr.ac.konkuk.ccslab.cm.manager.CMFileTransferManager.cancelPushFile(strUser, cmInfo);	// not clear
			}
		...
		*/
		...
		if(confInfo.isDBUse())
		{
			...
			*/
			...
			contentList = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryGetSNSContent(se.getUserName(), se.getWriterName(), nOffset, nContNum, cmInfo);
			contentVector = contentList.getContentList();
			...
			for(int i = nForStart; i < nForEnd; i++)
			{
				...
				if(nNumAttachedFiles > 0)
				{
					...
					attachFileList = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryGetSNSAttachedFile(nContID, cmInfo);
	...
	*/
	...
	private static void processCONTENT_DOWNLOAD_END_RESPONSE(CMSNSEvent se, CMInfo cmInfo)
	{
		...
		if(confInfo.getAttachDownloadScheme() == CMInfo.SNS_ATTACH_PREFETCH)
		{
			...
			for(int i = 0; i < preArrayList.size(); i++)
			{
				kr.ac.konkuk.ccslab.cm.manager.CMFileTransferManager.pushFile(preArrayList.get(i), se.getUserName(), cmInfo);
			}
		}
	...
	private static void processCONTENT_UPLOAD_REQUEST(CMSNSEvent se, CMInfo cmInfo)
	{
		...
		if( confInfo.isDBUse() )
		{
			...
			ret = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryInsertSNSContent(se.getUserName(), se.getMessage(), se.getNumAttachedFiles(),
					se.getReplyOf(), se.getLevelOfDisclosure(), cmInfo);
			if( ret == 1 )
			{
				// get the last insert ID
				String strQuery = "select last_insert_id() from sns_content_table;";
				ResultSet rs = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.sendSelectQuery(strQuery, cmInfo);
				try {
					if(rs != null && rs.next())
					{
						nSeqNum = rs.getInt(1); 
					...
					rs = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.sendSelectQuery(strQuery, cmInfo);
					...
					kr.ac.konkuk.ccslab.cm.manager.CMDBManager.closeDB(cmInfo);
					kr.ac.konkuk.ccslab.cm.manager.CMDBManager.closeRS(rs);
				}
			}
			else
			{
	...
	private static void processADD_NEW_FRIEND(CMSNSEvent se, CMInfo cmInfo)
	{
		...
		if(confInfo.isDBUse())
		{
			...
			try {
				...
				{
					// insert (user, friend)
					ret = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryInsertFriend(se.getUserName(), se.getFriendName(), cmInfo);
					if(ret == 1) // not clear
					{
						nReturnCode = 1;
					}
	...
	private static void processREMOVE_FRIEND(CMSNSEvent se, CMInfo cmInfo)
	{
		...
		if(confInfo.isDBUse())
		{
			...
			ret = kr.ac.konkuk.ccslab.cm.manager.CMDBManager.queryDeleteFriend(se.getUserName(), se.getFriendName(), cmInfo);
			if(ret == 1) // not clear
			{
				nReturnCode = 1;
			}
	...
	private static void processREQUEST_ATTACHED_FILES(CMSNSEvent se, CMInfo cmInfo)
	{
		...
		if(confInfo.getSystemType().equals("CLIENT"))
		{
			...
			{				
				kr.ac.konkuk.ccslab.cm.manager.CMFileTransferManager.pushFile(filePathList.get(i), se.getUserName(), CMInfo.FILE_DEFAULT,
		...
		else if(confInfo.getSystemType().equals("SERVER"))
		{
			...
			for(i = 0; i < filePathList.size(); i++)
			{
				...
				if(!file.exists())
				{
					...
					String strFileName = kr.ac.konkuk.ccslab.cm.manager.CMFileTransferManager.getFileNameFromPath(strFilePath);
					naFileNameList.add(strFileName);
				}
				else
				{
					kr.ac.konkuk.ccslab.cm.manager.CMFileTransferManager.pushFile(strFilePath, se.getUserName(), CMInfo.FILE_DEFAULT,
							se.getContentID(), cmInfo);
				}
				file = null;
			}
