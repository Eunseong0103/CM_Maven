...
public class CMGroupManager {
	...
	public static void init(CMInfo cmInfo) // for server
	{
		...
		while(iterSession.hasNext())
		{
			...
			while(iterGroup.hasNext())
			{
				...
				try {
					dc = (DatagramChannel) kr.ac.konkuk.ccslab.cm.manager.CMCommManager.openNonBlockChannel(CMInfo.CM_MULTICAST_CHANNEL,
				...
				MembershipKey key = kr.ac.konkuk.ccslab.cm.manager.CMCommManager.joinMulticastGroup(dc, group.getGroupAddress());
	...
	public static void changeGroup(String gName, CMInfo cmInfo) // for client
	{
		...
		kr.ac.konkuk.ccslab.cm.manager.CMEventManager.unicastEvent(ie, strDefServer, cmInfo);
		ie = null;
	...
	public static void processEvent(CMMessage msg, CMInfo cmInfo)
	{
		...
		// unmarshall an event
		cmEvent = kr.ac.konkuk.ccslab.cm.manager.CMEventManager.unmarshallEvent(msg.m_buf);
		if(cmEvent == null)
		{
	...
	private static void distributeGroupUsers(CMUser targetUser, CMInfo cmInfo)
	{
		...
		while(iterUser.hasNext())
		{
			...
			kr.ac.konkuk.ccslab.cm.manager.CMEventManager.unicastEvent(de, targetUser.getName(), cmInfo);
			de = null;
		}
	...
	private static void notifyGroupUsersOfNewUser(CMUser newUser, CMInfo cmInfo)
	{
		...
		kr.ac.konkuk.ccslab.cm.manager.CMEventManager.castEvent(de, gMember, cmInfo);
	...
	private static void notifyGroupUsersOfRemovedUser(CMUser user, CMInfo cmInfo)
	{
		...
		kr.ac.konkuk.ccslab.cm.manager.CMEventManager.castEvent(de,  group.getGroupUsers(), cmInfo);
	...
	public static void addDistributeGroupUsers(CMUser targetUser, CMInfo cmInfo)
	{
		...
		while(iterUser.hasNext())
		{
			...
			kr.ac.konkuk.ccslab.cm.manager.CMEventManager.unicastEvent(mse, targetUser.getName(), cmInfo);
			mse = null;
		}
