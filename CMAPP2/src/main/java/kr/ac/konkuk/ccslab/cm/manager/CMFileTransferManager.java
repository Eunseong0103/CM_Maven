...
public class CMFileTransferManager {
			...
			CMInfo cmInfo)
	{
		...
		if(isP2PFileTransfer(feAck, cmInfo))
		{
			...
			if(nReturnCode == 1 && confInfo.isFileTransferScheme())
			{
				...
				CMUser fileReceiver = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findGroupMemberOfClient(fe.getFileReceiver(), cmInfo);
	...
	private static boolean cancelPullFileWithSepChannelForOneSender(String strFileSender, CMInfo cmInfo)
	{
		...
		*/
		...
		CMServer targetServer = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findServer(strFileSender, cmInfo);
		...
		else
		{
			...
			{
				targetUser = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findGroupMemberOfClient(strFileSender,
						cmInfo);
			}
			else
			{
			...
			byte byteFileAppend, int nContentID, CMInfo cmInfo)
	{
		...
		*/
		...
		CMServer targetServer = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findServer(strFileReceiver, cmInfo);
		if(targetServer != null)
		{
		...
		else
		{
			...
			if(confInfo.getSystemType().contentEquals("CLIENT"))
			{
				targetUser = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findGroupMemberOfClient(strFileReceiver, cmInfo);
	...
	private static boolean cancelPushFileWithSepChannelForOneReceiver(String strFileReceiver, CMInfo cmInfo)
	{
		...
		*/
		...
		else
		{
			...
			if(confInfo.getSystemType().contentEquals("CLIENT"))
			{
				targetUser = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findGroupMemberOfClient(strFileReceiver,
						cmInfo);
			}
			else
			{
	...
	private static boolean processEND_FILE_TRANSFER(CMFileEvent fe, CMInfo cmInfo)
	{
		...
		kr.ac.konkuk.ccslab.cm.manager.CMFileSyncManager syncManager = cmInfo.getServiceManager(kr.ac.konkuk.ccslab.cm.manager.CMFileSyncManager.class);
	...
	private static boolean processSTART_FILE_TRANSFER_CHAN(CMFileEvent fe, CMInfo cmInfo)
	{
		...
		{
			CMServer serverInfo = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findServer(fe.getFileSender(), cmInfo);
			if(serverInfo != null)
			{
			...
			{
				CMUser targetUser = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findGroupMemberOfClient(fe.getFileSender(), cmInfo);
				if(targetUser == null)
				{
	...
	private static boolean processEND_FILE_TRANSFER_CHAN(CMFileEvent fe, CMInfo cmInfo)
	{
		...
		// check if the transfer is for sync a new file
		kr.ac.konkuk.ccslab.cm.manager.CMFileSyncManager syncManager = cmInfo.getServiceManager(CMFileSyncManager.class);
		syncManager.checkNewTransferForSync(fe);
		// check if the transfer is for the file-sync local mode
		syncManager.checkTransferForLocalMode(fe);
	...
	private static boolean processCANCEL_FILE_SEND_CHAN(CMFileEvent fe, CMInfo cmInfo)
	{
		...
		if(confInfo.getSystemType().equals("CLIENT"))
		{
			if(bP2PFileTransfer)
			{
				...
				CMUser targetUser = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findGroupMemberOfClient(
			...
			else
			{
				...
				CMServer server = kr.ac.konkuk.ccslab.cm.manager.CMInteractionManager.findServer(strFileSender,
						cmInfo);
				if(server == null)
				{
					System.err.println("CMFileTransferManager.processCANCEL_FILE_SEND_CHAN()"
